...
export default function GetNfts({ sdk, setRefresh, refresh }) {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const chain = "0x89";
        const response = await axios.get(`https://backened-asad-ghouri.vercel.app/getnfts`, {
          params: { address, chain },
        });
        setNfts(response.data.result);
      } catch (error) {
  ...
  return (
    <>
      ...
      {stakeNft && <StakeModal stakeNft={stakeNft} setStakeNft={setStakeNft} setRefresh={setRefresh} />}
    </>
  );
}

function StakeModal({ stakeNft, setStakeNft, setRefresh }) {
  const [stakingPeriod, setStakingPeriod] = useState(12);
  const [rewards, setRewards] = useState(null);
  const stakingContract = useContract(STAKING_CONTRACT_ADDRESS, STAKING_CONTRACT_ABI);
  const erc1155 = useContract(ERC1155_ADDRESS, ERC1155_ABI);
  const address = useAddress();
  ...
  useEffect(() => {
    async function fetchRewards() {
      try {
        ...
        const data = await stakingContract.contract.call("calculateRewards", [1, tokenPrice, stakingPeriod]);
        const act = ethers.utils.formatEther(data[0]);
        const usdt = ethers.utils.formatEther(data[1]) * 10 ** 12;
        setRewards({ act, usdt });
      } catch (error) {
  ...
  const getTokenPrice = async () => {
    try {
      const price = await stakingContract?.contract.call("getNFTPurchasingPrice", [stakeNft.token_id]);
      return parseInt(ethers.utils.formatEther(price) * 10 ** 18);
    } catch (error) {
  ...
  const stake = async () => {
    try {
      const approval = await erc1155?.contract?.call("isApprovedForAll", [address, STAKING_CONTRACT_ADDRESS]);
      if (!approval) await erc1155?.contract?.call("setApprovalForAll", [STAKING_CONTRACT_ADDRESS, true]);

      await stakingContract?.contract?.call("stake", [stakeNft.token_id, 1, stakingPeriod]);
  ...
  return (
    <div className="fixed top-0 left-0 w-full h-screen bg-[#000000be] z-[999999999999999] ">
      <div className="mx-auto mt-[100px] rounded-2xl p-5 bg-[#121212] relative w-[50%] md:w-[80%] sm:w-[90%] sm:flex-col sm:mt-[40px]  gap-5 justify-start items-center w30 zid" style={{ display: 'flex' }}>
        <h2 className="absolute top-0 right-5 cursor-pointer xcros" onClick={() => setStakeNft(false)}>X</h2>
        {stakeNft.token_id >= 0 && <img src={`https://i.seadn.io/s/raw/files/${stakeNft.image}`} width={300} height={400} className="w-full sm:h-[200px] object-cover" />}
        <div className="flex-col gap-2 w-full" style={{ display: "flex" }}>
          <h3 className="text-2xl font-bold">{stakeNft.name}</h3>
          <h4>Stake For</h4>
          <div className="gap-2 w-full" style={{ display: "flex" }}>
            {[12, 6, 3].map((months) => (
              <span key={months} onClick={() => setStakingPeriod(months)} className={`justify-center items-center gap-3 ${stakingPeriod === months ? 'bg-pink-400' : 'bg-black'} p-2 flex-1 rounded-md cursor-pointer zb`} style={{ display: "flex" }}>
                {months} Months
              </span>
            ))}
          </div>
